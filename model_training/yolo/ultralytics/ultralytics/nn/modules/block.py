...
# ==========================================
# 植入的 MobileNetV3 核心模块 (用于毕业设计轻量化)
# ==========================================
import torch.nn.functional as F


class SeBlock(nn.Module):
    """Squeeze-and-Excitation 注意力机制模块"""

    def __init__(self, in_planes):
        super().__init__()
        self.squeeze = nn.AdaptiveAvgPool2d(1)
        self.compress = nn.Conv2d(in_planes, in_planes // 4, 1, 1, 0)
        self.excitation = nn.Conv2d(in_planes // 4, in_planes, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        # MobileNetV3 使用 h-sigmoid
        return x * F.hardtanh(out + 3, 0., 6.) / 6.


class Bneck(nn.Module):
    """MobileNetV3 的核心倒残差模块"""

    def __init__(self, c1, c2, e, k, s, use_se, act_func):
        super().__init__()
        self.use_res_connect = s == 1 and c1 == c2
        layers = []
        # 1. 升维 (Expand)
        if e != c1:
            layers.append(Conv(c1, e, 1, 1, act=act_func))
        # 2. 深度可分离卷积 (Depthwise)
        layers.append(Conv(e, e, k, s, g=e, act=act_func))
        # 3. SE 注意力机制
        if use_se:
            layers.append(SeBlock(e))
        # 4. 降维输出 (Project) 线性激活
        layers.append(Conv(e, c2, 1, 1, act=False))
