...
def verify_image(args: tuple) -> tuple:
    ...
    nf, nc, msg = 0, 0, ""
    try:
        # ========== 核心修改：新增对 4D .npy 文件的拦截支持 ==========
        if str(im_file).endswith('.npy'):
            import numpy as np
            # 尝试加载 npy 并获取形状 (H, W)，如果加载失败会自然走到 except 报错
            shape = np.load(im_file).shape[:2]
...
def verify_image_label(args: tuple) -> list:
    ...
    try:
        ...
        # Verify labels (这部分完全保持官方原样，负责扫描你的 .txt 标签)
