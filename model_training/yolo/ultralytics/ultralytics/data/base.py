    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        ...
        if im is None:  # not cached in RAM
            ...
            if str(f).endswith('.npy'):
                # 1. 如果你自己的原始数据集文件就是 .npy (4D 数据)，直接加载
                # 这里避开了官方的 except 删除逻辑，绝对保护你的数据集安全
                im = np.load(f)
            else:
                # 2. 官方原版的加载逻辑 (处理传统的 jpg/png 及其对应的 npy 缓存)
                if fn.exists():  # load npy cache
                    try:
                        im = np.load(fn)
                    except Exception as e:
            ...
            # 下面的 resize 逻辑完全不需要动
            # 因为 OpenCV (cv2.resize) 原生支持对 4 通道 (RGB-D) 数组进行缩放！
